...
namespace our
{
    ...
    void ForwardRenderer::initialize(glm::ivec2 windowSize, const nlohmann::json &config)
    {
        ...
        if (config.contains("postprocess"))
        {
            ...
            // Constructing color and depth textures
            colorTarget = new Texture2D();
            depthTarget = new Texture2D();
            // Binding color and depth textures
            colorTarget->bind();
            depthTarget->bind();
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, windowSize.x, windowSize.y, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, NULL);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            ...
            // Attaching depth texture to the framebuffer
    ...
    void ForwardRenderer::render(World *world)
    {
        ...
        if (postprocessMaterial)
        {
            ...
            // glGenFramebuffers(1, &postprocessFrameBuffer);
            glBindFramebuffer(GL_TEXTURE_2D, postprocessFrameBuffer);
        }
