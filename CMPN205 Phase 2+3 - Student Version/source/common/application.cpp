...
int our::Application::run(int run_for_frames) {
  ...
  auto win_config =
      getWindowConfiguration();  // Returns the WindowConfiguration current
                                 // struct instance.
  ...
  glfwMakeContextCurrent(
      window);  // Tell GLFW to make the context of our window the main context
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    last_frame_time =
        current_frame_time;  // Then update the last frame start time (this
                             // frame is now the last frame)
    ...
    if (player1_score >= 5 || player2_score >= 5) {
      if (player1_score >= 5) winner = 1;
      if (player2_score >= 5) winner = 2;
      player1_score = 0;
      player2_score = 0;
      changeState("winner");
      collision = 1;
...
void our::Application::setupCallbacks() {
  ...
  glfwSetCursorPosCallback(
      window, [](GLFWwindow* window, double x_position, double y_position) {
        auto* app = static_cast<Application*>(glfwGetWindowUserPointer(window));
        if (app) {
